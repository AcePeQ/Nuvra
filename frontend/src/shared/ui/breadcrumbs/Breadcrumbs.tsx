...
import { ROOT_URL_SHORTER } from "../../utils/helpers";

function splitPath(path: string) {
  return path.split("/");
}

function forgePath(path: string, index: number) {
...
function Breadcrumbs() {
  ...
  const breadcrumbs = splitPath(pathname).filter(Boolean);

  return (
    <ol className={styles.breadcrumbs}>
      <li className={styles.breadcrumbs__item}>
        <Link className={styles.breadcrumbs__link} to={ROOT_URL_SHORTER}>
          Home
